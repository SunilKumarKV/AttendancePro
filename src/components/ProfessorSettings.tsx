...
export const ProfessorSettings: React.FC = () => {
    ...
    <div className="mx-auto max-w-5xl pb-12">
      <div className="mb-8 flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-600 dark:text-slate-300">Manage account preferences available to professor accounts.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
        <button type="button" onClick={() => navigate('/professor-profile')} className="rounded-3xl border border-slate-100 bg-white p-6 text-left shadow-sm transition-colors hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:hover:bg-slate-900">
      ...
      <div className="mt-8">
        <EmptyState title="No professor-only institution settings" message="Institution-wide settings are controlled by administrators to keep academic rules consistent." />
      </div>
    </div>
  );
};
